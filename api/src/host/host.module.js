"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.HostModule = void 0;
const common_1 = require("@nestjs/common");
const controllers_1 = require("./controllers");
const manager_module_1 = require("../manager/manager.module");
let HostModule = class HostModule {
};
exports.HostModule = HostModule;
exports.HostModule = HostModule = __decorate([
    (0, common_1.Module)({
        imports: [manager_module_1.ManagerModule],
        controllers: [
            controllers_1.CashBoxController,
            controllers_1.GroupsController,
            controllers_1.MembersController,
            controllers_1.ParallelLoansController,
            controllers_1.PaymentsController,
            controllers_1.RuedasController,
        ],
    })
], HostModule);
//# sourceMappingURL=host.module.js.map